# Dockerfile fork from https://github.com/blocknetdx/dockerimages.git branch lightpaycoin-v6.16.5.1
# Build via docker:
...
RUN mkdir -p /opt/blocknet \
  && cd /opt/blocknet \
  && git clone --depth 1 --branch $VER https://github.com/lpcproject/LightPayCoin repo 
...
RUN mkdir -p /opt/blockchain/config \
  ...
  && make -j$ecores \
  && strip src/lightpaycoind \
  && strip src/lightpaycoin-tx \
  && strip src/lightpaycoin-cli \
  && make install 
...
COPY --from=builder /bin/lightpaycoin* /usr/local/bin/
